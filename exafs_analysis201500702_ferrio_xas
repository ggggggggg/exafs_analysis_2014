...
for ds in data:
    ds.p_energy[:] = ds.p_filt_value[:]/np.median(ds.p_filt_value[ds.good()])


plt.figure()
bin_edges = np.arange(0,2,0.005)
bin_centers = bin_edges[0]*0.5+bin_edges[0:-1]
fitr = np.arange(150,200)
for ds in data:
    counts, bins = np.histogram(ds.p_energy[ds.cuts.good()], bin_edges)
    # plt.hist(ds.p_energy[ds.good()], bin_edges, histtype="step")
    exafs.fit_edge_hist(bin_centers[fitr], counts[fitr], 2)
