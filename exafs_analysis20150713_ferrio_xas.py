...
dir_p = "20150713_ferrio_500mM_xas"
...
crate_unix_usecs, crate_frames = load_aux_file(os.path.join(dir_base, dir_p, "20150713_ferrio_500mM_xas"))
...
def get_nonoverlapping_ranges(ranges):
    ...
    while True:
        r = ordered_ranges.pop(0)

        try:
            s = ordered_range
        except IndexError:
            nonoverlapping_ranges.append(r)
            break
...
matter_badranges = find_badranges((unix_timestamp_bins[1:] + unix_timestamp_bins[:-1])/2,
                                  matter_counts, threshold_power_hi=4700, threshold_power_lo=100)
...
ax.set_xlabel("Energy (eV)")
ax.set_ylabel(r"Count ($\times$ 1000 per 2 eV bin)")
ax.set_title("20150713 ferrioxalate 500 mM with Blue power and Matter count cuts")
...
# Write to files.
###########################################################
with open("20150713_pumped_hist_0.5ev_bin_blue_matter_cut.dat", 'w') as f:
    for e, c in zip((bins[1:] + bins[:-1])/2, pumped_hist_full):
        f.write("{0:.2f},{1:d}\n".format(e, int(c)))

with open("20150713_unpumped_hist_0.5ev_bin_blue_matter_cut.dat", 'w') as f:
