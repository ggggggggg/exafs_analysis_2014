...
dir_p = "20141027_ferioxalate_1p2M"
dir_n = "20141027_ferioxalate_1p2M_noise"
...
exafs.quality_control_range(data, exafs.edge_drop_func, "edge drop", range=(0.02,2.0))
...
tstep = t[1]-t[0]
pt_mean = np.zeros_like(t)
pt_std = np.zeros_like(t)
for i in range(len(t)):
    use = np.logical_and(np.abs(ds.p_timestamp-t[i])<tstep*0.5, ds.cuts.good())
    if np.sum(use) == 0:
        continue
    else:
        pt_mean[i] = np.mean(ds.p_pretrig_mean[use])
        pt_std[i] = np.std(ds.p_pretrig_mean[use])

plt.figure()
plt.plot(t,(r-np.mean(r))/np.std(r),'.')
pfit = np.polyfit(t,pt_mean,1)
pt_mean2 = pt_mean-np.polyval(pfit,t)
plt.plot(t,(pt_mean-np.mean(pt_mean))/np.std(pt_mean),'.')

plt.figure()
plt.plot(r*240, pt_mean2,'.')
plt.xlabel("approx array trigger rate (trigger/s)")
plt.ylabel("pt mean variation after linear in time part")
