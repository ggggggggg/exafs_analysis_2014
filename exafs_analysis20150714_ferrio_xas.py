...
dir_p = "20150714_ferrio_500mM_xas"
dir_n = "20150714_ferrio_500mM_xas_noise"
...
ax.hist(ds.p_timestamp, bins=np.linspace(55000, 85000, 30001), histtype='step')
...
crate_unix_usecs, crate_frames = load_aux_file(os.path.join(dir_base, dir_p, "20150714_ferrio_500mM_xas"))
...
unix_timestamp_bins = np.arange(
    int(np.min(ds.p_unix_timestamp)) / 1000 * 1000,
    int(np.max(ds.p_unix_timestamp)) / 1000 * 1000 + 1001,
    1
)
...
matter_badranges = find_badranges((unix_timestamp_bins[1:] + unix_timestamp_bins[:-1])/2,
                                  matter_counts, threshold_power_hi=4750, threshold_power_lo=100)
...
badranges_prepped_for_cut = total_badranges
badranges_prepped_for_cut.append("invert")
...
ax.set_title("20150714 500mM ferrioxalate with Blue power and Matter count cuts")
...
ax.set_xlabel("Energy (eV)")
ax.set_ylabel(r"Count ($\times$ 1000 per 2 eV bin)")
ax.set_title("20150714 ferrioxalate 500 mM with Blue power and Matter count cuts")
...
# Write to files.
###########################################################
with open("20150714_pumped_hist_0.5ev_bin_blue_matter_cut.dat", 'w') as f:
    for e, c in zip((bins[1:] + bins[:-1])/2, pumped_hist_full):
        f.write("{0:.2f},{1:d}\n".format(e, int(c)))

with open("20150714_unpumped_hist_0.5ev_bin_blue_matter_cut.dat", 'w') as f:
