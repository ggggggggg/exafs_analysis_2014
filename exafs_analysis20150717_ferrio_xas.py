...
dir_p = "20150717_500mM_ferrio_xas"
...
crate_unix_usecs, crate_frames = load_aux_file(os.path.join(dir_base, dir_p, "20150717_500mM_ferrio_xas"))
...
with open("vega_power_20150720.dat") as vega_file:
    for l in vega_file:
...
unix_timestamp_ranges = [(np.min(ds.p_unix_timestamp), np.max(ds.p_unix_timestamp)) for ds in data]

unix_timestamp_bins = np.arange(
    int(np.min([r[0] for r in unix_timestamp_ranges])) / 1000 * 1000,
    int(np.max([r[1] for r in unix_timestamp_ranges])) / 1000 * 1000 + 1000,
    1
)
...
matter_badranges = find_badranges((unix_timestamp_bins[1:] + unix_timestamp_bins[:-1])/2,
                                  matter_counts, threshold_power_hi=2600, threshold_power_lo=100)
...
ax.set_title("20150717 500mM ferrioxalate with Blue power and Matter count cuts")
...
ax.set_xlabel("Energy (eV)")
ax.set_ylabel(r"Count ($\times$ 1000 per 2 eV bin)")
ax.set_title("20150717 ferrioxalate 500 mM with Matter count cuts")
...
# Write to files.
###########################################################
with open("20150717_pumped_hist_0.5ev_bin_blue_matter_cut.dat", 'w') as f:
    for e, c in zip((bins[1:] + bins[:-1])/2, pumped_hist_full):
        f.write("{0:.2f},{1:d}\n".format(e, int(c)))

with open("20150717_unpumped_hist_0.5ev_bin_blue_matter_cut.dat", 'w') as f:
