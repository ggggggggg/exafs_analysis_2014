...
dir_p = "20150720_500mM_ferrio_xas"
dir_n = "20150720_500mM_ferrio_xas_noise"
...
crate_unix_usecs, crate_frames = load_aux_file(os.path.join(dir_base, dir_p, "20150720_500mM_ferrio_xas"))
...
#############
#  Plotting vega powers.
#######
vega_timestamps = []
vega_powers = []

with open("vega_power_20150723.dat") as vega_file:
    for l in vega_file:
...
ds = data.channel[5]
ax.set_xlim(datetime.fromtimestamp(ds.timestamp_offset + np.min(ds.p_timestamp) - 60, tz=mtz),
            datetime.fromtimestamp(ds.timestamp_offset + np.max(ds.p_timestamp) - 60, tz=mtz))

fig.show()

# Create p_unix_timestamp data set for each channel.
for ds in data:
    pulse_unix_timestamp = pulse_timing.extrap(ds.p_timestamp[...] / 9.6e-6, crate_frames,
                                               1.0e-6 * crate_unix_usecs)
...
############################
# Calculated tes array count rate
############################

min_unix_timestamp = np.min([np.min(ds.p_unix_timestamp) for ds in data])
max_unix_timestamp = np.max([np.max(ds.p_unix_timestamp) for ds in data])

unix_timestamp_bins = np.arange(
    int(min_unix_timestamp) / 1000 * 1000,
    int(max_unix_timestamp) / 1000 * 1000 + 1000,
    1
)
...
for ds in data:
    hist, bins = np.histogram(ds.p_unix_timestamp, bins=unix_timestamp_bins)
    matter_counts += hist

for ds in data:
    pulse_matter_count = pulse_timing.extrap(ds.p_unix_timestamp[...],
...
vega_badranges = find_badranges(vega_timestamps, vega_powers, threshold_power_hi=0.3, threshold_power_lo=0.2)
matter_badranges = find_badranges((unix_timestamp_bins[1:] + unix_timestamp_bins[:-1])/2,
                                  matter_counts, threshold_power_hi=5500, threshold_power_lo=100)
...
for ds in data:
    ds.cuts.clearCut(CUT_NUM)
    ds.cut_parameter(ds.p_unix_timestamp[...], badranges_prepped_for_cut, CUT_NUM)
...
ax.set_title("20150720 500mM ferrioxalate with Blue power and Blue power and Matter count cuts")

#ax.set_ylim(20.5, 33.5)
...
bins = np.linspace(2000, 15000, 26001)
...
ax.set_xlabel("Energy (eV)")
ax.set_ylabel(r"Count ($\times$ 1000 per 2 eV bin)")
ax.set_title("20150720 ferrioxalate 500 mM with Blue power and Matter count cuts")
...
# Write to files.
###########################################################
with open("20150720_pumped_hist_0.5ev_bin_blue_matter_cut.dat", 'w') as f:
    for e, c in zip((bins[1:] + bins[:-1])/2, pumped_hist_full):
        f.write("{0:.2f},{1:d}\n".format(e, int(c)))

with open("20150720_unpumped_hist_0.5ev_bin_blue_matter_cut.dat", 'w') as f:
    for e, c in zip((bins[1:] + bins[:-1])/2, unpumped_hist_full):
        f.write("{0:.2f},{1:d}\n".format(e, int(c)))

with open("20150715_ferrioxlate_pumped_coadded.dat", 'w') as f:
    for e, c in zip(np.linspace(2001, 14999, 6500), shared_pumped):
        f.write("{0:.2f},{1:d}\n".format(e, c))

with open("20150715_ferrioxlate_unpumped_coadded.dat", 'w') as f:
